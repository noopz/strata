...
function makeTree(filePath, children, lineCount) {
    return {
        filePath,
        mtime: Date.now(),
...
describe("CrossFileIndex", () => {
    ...
    describe("connection detection", () => {
        it("should detect API dependency when header in A matches body in B", () => {
            ...
            const apiConnA = connsA.find((c) => c.type === "api_dependency" && c.targetFile === "/b.ts");
            assert.ok(apiConnA, "should detect API dependency from A to B");
            assert.equal(apiConnA.direction, "outgoing");
            assert.ok(apiConnA.sharedTokens.includes("builddynamiccontext"), "shared tokens should include the full compound token");
            // From B's perspective: B references, A defines → incoming
            const connsB = index.getConnections("/b.ts");
            const apiConnB = connsB.find((c) => c.type === "api_dependency" && c.targetFile === "/a.ts");
        ...
        it("should detect shared interface when both files define token in headers", () => {
            ...
            const shared = conns.find((c) => c.type === "shared_interface" && c.targetFile === "/b.ts");
        ...
        it("should detect conceptual coupling when token in body of both files", () => {
            ...
            index.indexFile("/b.ts", treeB, linesB);
            const conns = index.getConnections("/a.ts");
            const coupling = conns.find((c) => c.type === "conceptual_coupling" && c.targetFile === "/b.ts");
    ...
    describe("IDF filtering of common tokens", () => {
        it("should filter out tokens appearing in more than half the files", () => {
            ...
            for (const conn of conns) {
                assert.ok(!conn.sharedTokens.includes("log"), "'log' should be filtered out (appears in all files)");
            }
        ...
        it("should keep tokens in the IDF sweet spot", () => {
            ...
            assert.ok(conn.sharedTokens.includes("raresymbol"), "shared tokens should include 'raresymbol'");
            // ubiquitous should not appear in any connection's shared tokens
            for (const c of conns) {
                assert.ok(!c.sharedTokens.includes("ubiquitous"), "'ubiquitous' should be filtered (appears in all files)");
            }
        });
    });
    describe("file removal", () => {
        it("should clean up all index entries when a file is removed", () => {
            ...
            assert.deepEqual(index.getIndexedFiles(), [], "no files should remain after removal");
            assert.equal(index.getTokenStats("uniquefunction"), undefined, "token stats should be gone after file removal");
        });
    ...
    describe("re-indexing a file", () => {
        it("should replace old tokens when re-indexing the same file", () => {
            ...
            assert.equal(index.getTokenStats("oldname"), undefined, "old token should be removed");
            assert.ok(index.getTokenStats("newname"), "new token should be present");
        });
    });
    describe("connection strength ordering", () => {
        it("should rank connections by strength descending", () => {
            ...
            if (conns.length >= 2) {
                ...
                if (bConn && cConn) {
                    assert.ok(bConn.strength >= cConn.strength, "B (sharing more tokens) should have >= strength than C");
                }
            }
        });
    });
...
//# sourceMappingURL=cross-file-index.test.js.map