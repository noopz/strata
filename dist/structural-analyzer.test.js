...
import { analyzeFile, shannonEntropy, jaccardSimilarity, indentDepth, bracketDelta, computeSmoothedEntropy, entropyGradientAt, } from "./structural-analyzer.js";
// ---------------------------------------------------------------------------
...
describe("jaccardSimilarity", () => {
    ...
    it("returns value between 0 and 1 for partially similar strings", () => {
        const sim = jaccardSimilarity("function hello() {", "function world() {");
...
function another() {
  ...
  return x + y;
        ...
        assert.ok(tree.children.length >= 2, `expected >= 2 children, got ${tree.children.length}`);
    ...
    it("collapses repetitive regions", () => {
        ...
        assert.ok(totalNodes < 30, `expected < 30 nodes with collapse, got ${totalNodes}`);
        // Check that at least one node is collapsed
        function hasCollapsed(nodes) {
            for (const n of nodes) {
                if (n.collapsed)
                    return true;
                if (hasCollapsed(n.children))
                    return true;
            }
            return false;
        }
        assert.ok(hasCollapsed(tree.children), "expected at least one collapsed node");
    });
    ...
    for (const item of items) {
      await this.processRequest({ id: item } as Request);
        ...
        if (tree.children.length > 0) {
            assert.equal(tree.children[0].startLine, 1);
            assert.equal(tree.children[tree.children.length - 1].endLine, content.length);
        }
    });
...
function lastOne() {
  ...
  return a + b;
        ...
        function maxTreeDepth(nodes, currentDepth) {
            if (nodes.length === 0)
                return currentDepth;
...
export function calculateTotal(items: Item[]): number {
  ...
  return total;
        ...
        function findLabel(nodes, substr) {
            for (const n of nodes) {
                if (n.label.includes(substr))
                    return true;
                if (findLabel(n.children, substr))
                    return true;
            }
            return false;
        }
        assert.ok(findLabel(tree.children, "calculateTotal") ||
    ...
    it("truncates labels to 80 characters", () => {
        const longLine = "export function thisIsAVeryLongFunctionNameThatShouldBeTruncatedBecauseItExceedsTheMaximumLabelLength(param1: string, param2: number): boolean {";
        const lines = [longLine, "  return true;", "}"];
        const tree = analyzeFile(lines, "/test/long.ts", 1000);
        assert.ok(tree.children.length >= 1);
        assert.ok(tree.children[0].label.length <= 80, `label too long: ${tree.children[0].label.length}`);
    });
...
export function realContent() {
  return true;
        ...
        function findNodeWithLabel(nodes, substr) {
            for (const n of nodes) {
                if (n.label.includes(substr))
                    return n;
        ...
        function checkLabelLine(nodes) {
            for (const n of nodes) {
                assert.ok(typeof n.labelLine === "number" && n.labelLine >= 1, `node ${n.label} missing valid labelLine, got ${n.labelLine}`);
...
describe("computeSmoothedEntropy", () => {
    it("returns smoothed values for a simple profile", () => {
        ...
        const smoothed = computeSmoothedEntropy(profiles, 0, 5);
    ...
    it("handles subregions correctly", () => {
        ...
        // Only analyze [3, 7)
        const smoothed = computeSmoothedEntropy(profiles, 3, 7);
...
describe("entropy-adaptive collapse", () => {
    it("uses dynamic threshold to prevent collapsing regions with different entropy", () => {
        ...
        function findCollapsedCoveringAll(nodes) {
            for (const n of nodes) {
                if (n.collapsed && n.startLine === 1 && n.endLine === lines.length) {
                    return true;
                }
                if (findCollapsedCoveringAll(n.children))
                    return true;
            }
            return false;
        }
        assert.ok(!findCollapsedCoveringAll(tree.children), "entropy guard should prevent collapsing simple + complex regions together");
    });
});
// ---------------------------------------------------------------------------
...
describe("entropy-weighted boundary scoring", () => {
    it("splits at entropy transition between structurally different sections", () => {
        ...
        assert.ok(firstEnd >= 19 && firstEnd <= 22, `expected first section to end near line 21, got ${firstEnd}`);
    });
});
// ---------------------------------------------------------------------------
...
describe("entropy-driven boundary detection", () => {
    it("decomposes large regions with distinct entropy zones via entropy boundaries", () => {
        ...
        assert.ok(tree.children.length >= 2, `expected >= 2 top-level sections, got ${tree.children.length}`);
...
//# sourceMappingURL=structural-analyzer.test.js.map