...
export function shannonEntropy(line) {
    if (line.length === 0)
        return 0;
    const freq = new Map();
...
export function indentDepth(line) {
    let depth = 0;
    for (let i = 0; i < line.length; i++) {
        if (line[i] === " ")
            depth++;
 ...
 */
export function tagDelta(line) {
    if (TAG_SELF_CLOSE_RE.test(line))
        return 0;
    if (TAG_CLOSE_RE.test(line))
        return -1;
    if (TAG_OPEN_RE.test(line))
        return 1;
    return 0;
}
function computeProfiles(lines) {
...
function runningBracketDepth(profiles, start, end) {
    const depths = [];
    let depth = 0;
...
function runningTagDepth(profiles, start, end) {
    const depths = [];
    let depth = 0;
    for (let i = start; i < end; i++) {
 ...
 */
export function computeSmoothedEntropy(profiles, start, end, window) {
 ...
 */
export function entropyGradientAt(smoothed, localIdx) {
    if (localIdx <= 0 || localIdx >= smoothed.length)
        return 0;
 ...
 */
function findExplicitBoundaries(profiles, lines, start, end) {
    const bounds = [];
 ...
 */
function findEntropyTransition(profiles, start, end) {
    const regionLen = end - start;
    if (regionLen < ENTROPY_WINDOW * 2 + 1)
        return -1;
 ...
 */
function findEntropyBoundaries(profiles, lines, start, end) {
    const regionLen = end - start;
    if (regionLen < MIN_REGION * 3)
        return [];
    ...
    if (regionEntropies.length > 0) {
        const rMean = regionEntropies.reduce((a, b) => a + b, 0) / regionEntropies.length;
        if (rMean > 0) {
            const rVar = regionEntropies.reduce((a, b) => a + (b - rMean) ** 2, 0) /
    ...
    for (const peak of peaks) {
        if (kept.length >= maxSplits)
            break;
        const tooClose = kept.some((k) => Math.abs(k.localIdx - peak.localIdx) < minGap);
        if (!tooClose) {
            kept.push(peak);
        }
    }
...
function extractLabel(lines, start, end) {
    for (let i = start; i < end; i++) {
...
// Region statistics
// ---------------------------------------------------------------------------
function regionAvgEntropy(profiles, start, end) {
    if (end <= start)
        return 0;
    let sum = 0;
    for (let i = start; i < end; i++) {
...
function regionAvgSimilarity(lines, start, end) {
    if (end - start <= 1)
        return 1;
    let sum = 0;
    let count = 0;
 ...
 */
function collapseConsecutiveSiblings(nodes, lines, eStats) {
    if (nodes.length < MIN_COLLAPSE_RUN)
        return nodes;
    const result = [];
    let runStart = 0;
    while (runStart < nodes.length) {
        ...
        while (runEnd < nodes.length) {
            ...
            const sizeRatio = Math.min(prevSize, currSize) / Math.max(prevSize, currSize);
            ...
            const z = eStats.std > ENTROPY_MIN_STD
                ? (pairEntropy - eStats.mean) / eStats.std
                : 0;
            const dynamicThreshold = Math.max(0.5, Math.min(0.9, SIMILARITY_COLLAPSE_THRESHOLD + z * ENTROPY_COLLAPSE_SCALE));
 ...
 */
function mergeClosingDelimiters(nodes) {
    if (nodes.length <= 1)
        return nodes;
    const result = [];
...
// ---------------------------------------------------------------------------
function analyzeRegion(lines, profiles, start, end, depth, maxDepth, eStats) {
    const regionLen = end - start;
    ...
    if (usable.length > 0) {
        ...
        if (usable.length > maxBoundaries && regionLen >= 200) {
            if (depth === 0 && regionLen >= targetSize * 3) {
                ...
                const coarseSmoothed = computeSmoothedEntropy(profiles, start, end, coarseWindow);
                const coarseGrads = [];
                for (let j = 0; j < coarseSmoothed.length; j++) {
                    coarseGrads.push(entropyGradientAt(coarseSmoothed, j));
                }
                coarseGrads.sort((a, b) => a - b);
                const cScale = coarseGrads[Math.floor(coarseGrads.length * 0.9)] ?? 0.1;
    ...
    for (const [segS, segE] of segments) {
        const sub = analyzeRegion(lines, profiles, segS, segE, depth + 1, maxDepth, eStats);
        if (sub.length > 1) {
...
function makeLeaf(lines, profiles, start, end) {
...
// ---------------------------------------------------------------------------
function defaultMaxDepth(lineCount) {
    if (lineCount < 300)
        return 2; // enough for heading-structured markdown
    if (lineCount <= 2000)
        return 4;
    if (lineCount <= 50000)
        return 3;
 ...
 */
export function analyzeFile(lines, filePath, mtime, maxDepth) {
    ...
    if (nonBlankEntropies.length > 0) {
        ...
        const variance = nonBlankEntropies.reduce((a, b) => a + (b - eMean) ** 2, 0) /
            nonBlankEntropies.length;
        eStd = Math.sqrt(variance);
    }
    const eStats = { mean: eMean, std: eStd };
