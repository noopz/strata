...
# Write context to a temp file to avoid heredoc quoting issues
TMPCTX=$(mktemp)
trap 'rm -f "$TMPCTX"' EXIT

cat > "$TMPCTX" << 'CTXEOF'
Strata structural outlines are active for large files (300+ lines).
...
ESCAPED=$(node -e "
process.stdout.write(JSON.stringify(require('fs').readFileSync('$TMPCTX','utf8')).slice(1,-1));
" 2>/dev/null)
