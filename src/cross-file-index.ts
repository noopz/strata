...
export class CrossFileIndex {
  ...
  private tokenIndex: Map<string, TokenOccurrence[]> = new Map();
  /** Inverted index: filePath → Set of normalized tokens */
  private fileTokens: Map<string, Set<string>> = new Map();
  /** Cached stats: normalized token → TokenStats */
  private statsCache: Map<string, TokenStats> = new Map();
  /** Connection cache: filePath → CrossFileConnection[] */
  private connectionCache: Map<string, CrossFileConnection[]> = new Map();
  ...
  indexFile(filePath: string, tree: StructuralTree, lines: string[]): void {
    ...
    const walkNode = (node: StructuralNode): void => {
      ...
      for (const text of headerTokenTexts) {
        ...
        if (seenInHeader.has(normalized)) continue;
        ...
        for (const comp of components) {
          ...
          if (compNorm === normalized) continue;
          if (seenInHeader.has(compNorm)) continue;
      ...
      if (bodyStartLine <= bodyEndLine) {
        for (let lineIdx = bodyStartLine; lineIdx <= bodyEndLine; lineIdx++) {
          ...
          for (const text of bodyTokenTexts) {
            const normalized = text.toLowerCase();
            if (seenInBody.has(normalized)) continue;
            ...
            for (const comp of components) {
              const compNorm = comp.toLowerCase();
              if (compNorm === normalized) continue;
              if (seenInBody.has(compNorm)) continue;
  ...
  getConnections(filePath: string, limit?: number): CrossFileConnection[] {
    ...
    for (const token of tokens) {
      ...
      if (stats.fileCount < 2 || stats.fileCount > maxFileCount) continue;
      ...
      const otherFiles = new Map<
        string,
        { inHeader: boolean; inBody: boolean }
      >();
      for (const occ of allOccs) {
        if (occ.filePath === filePath) continue;
        let entry = otherFiles.get(occ.filePath);
        if (!entry) {
          entry = { inHeader: false, inBody: false };
          otherFiles.set(occ.filePath, entry);
        }
        if (occ.position === "header") entry.inHeader = true;
        if (occ.position === "body") entry.inBody = true;
      }
  ...
  exportState(): string {
    const tokenIndex: Record<string, TokenOccurrence[]> = {};
  ...
  importState(json: string): void {
    const data = JSON.parse(json) as {
      tokenIndex: Record<string, TokenOccurrence[]>;
  ...
  private computeStats(
    ...
    for (const occ of occs) {
      ...
      if (occ.position === "header") headerCount++;
      else bodyCount++;
    }
    ...
    const stats: TokenStats = { idf, fileCount, headerCount, bodyCount };
